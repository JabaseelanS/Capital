...
import { currentUserPermissions, Permission, AuthDataContext } from '../../../../core/auth';
...
export class BaseComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		// debugger
		var loginList = JSON.parse(localStorage.getItem("userdetails"));
		...
		// For Menus
		if (loginList.RoleId == 2) { // Pharmacy group admin
			this.menuConfigService.loadConfigs(new MenuConfig().pharmacyGrpAdmin);
		}
		else if (loginList.RoleId == 3) { // Pharmacy user
			this.menuConfigService.loadConfigs(new MenuConfig().pharmacyUser);
		}
		else if (loginList.RoleId == 4) { // Pharmacy test user
			this.menuConfigService.loadConfigs(new MenuConfig().pharmacyTestUser);
		}
		else if (loginList.RoleId == 5) { // pharmacy admin 
			this.menuConfigService.loadConfigs(new MenuConfig().pharmacyGrpUserAdmin);
		}
		else { // Packapill admin
			this.menuConfigService.loadConfigs(new MenuConfig().defaults);
		}
